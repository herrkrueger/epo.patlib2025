...
def build_ree_dataset(db, test_mode=True):
    ...
    WHERE (
        ...
        LOWER(t.appln_title) LIKE '%terbium%' OR
        LOWER(t.appln_title) LIKE '%europium%' OR
        LOWER(t.appln_title) LIKE '%yttrium%' OR
        ...
        LOWER(ab.appln_abstract) LIKE '%REE recycling%' OR
        LOWER(ab.appln_abstract) LIKE '%lanthanide%' OR
        LOWER(ab.appln_abstract) LIKE '%light REE%' OR
        LOWER(ab.appln_abstract) LIKE '%heavy REE%'
    )
    ...
    WHERE (
        ...
        cpc.cpc_class_symbol LIKE 'C04B18/04%' OR
        cpc.cpc_class_symbol LIKE 'C04B18/305%' OR
        cpc.cpc_class_symbol LIKE 'C09K11/01%' OR
        cpc.cpc_class_symbol LIKE 'H01J9/52%' OR
        cpc.cpc_class_symbol LIKE 'H01M6/52%' OR
        cpc.cpc_class_symbol LIKE 'H01M10/54%' OR
        cpc.cpc_class_symbol LIKE 'Y02W30/52%' OR
        cpc.cpc_class_symbol LIKE 'Y02W30/56%' OR
    ...
    if common_appln_ids:
        ...
        print("No intersection found, using keyword results as primary dataset")
        return keyword_results.head(500) if test_mode else keyword_results
